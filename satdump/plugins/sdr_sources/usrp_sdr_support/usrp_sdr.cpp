...
void USRPSource::open_channel()
{
    ...
    if (usrp_device->get_master_clock_rate_range().start() != usrp_device->get_master_clock_rate_range().stop()){
      ...
      logger->trace("get_master_clock_rate_range().start() = %d", usrp_device->get_master_clock_rate_range().start());
      logger->trace("get_master_clock_rate_range().stop() = %d", usrp_device->get_master_clock_rate_range().stop());
      logger->warn("Using device rates, master_clock_rate_range start and stop are different!");
    }

    uint64_t current_samplerate = get_samplerate();
    ...
    std::vector<double> int_samplerates;
    for (auto &sr : available_samplerates){
      if(static_cast<uint64_t>(sr) == sr) int_samplerates.push_back(sr);
    }

    samplerate_widget.set_list(int_samplerates, false);
    if (current_samplerate > 0) set_samplerate(current_samplerate);
...
{
  return static_cast<uint64_t>(samplerate_widget.get_value());
}
...
std::vector<dsp::SourceDescriptor> USRPSource::getAvailableSources()
{
    ...
    uhd::device_addrs_t devlist = uhd::device::find(uhd::device_addr_t());
    std::set<std::string> device_addresses{};

    uint64_t i = 0;
    for (const uhd::device_addr_t &dev : devlist)
    {
        if(device_addresses.find(dev["addr"]) != device_addresses.end()) continue;
        std::string type = dev.has_key("name") ? dev["name"] : dev["type"];
        results.push_back({"usrp", "USRP " + type + " " + dev["serial"] + "(" + dev["addr"] + ")", std::to_string(i)});
        device_addresses.insert(dev["addr"]);
        i++;
    }
