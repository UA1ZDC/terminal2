...
namespace satdump
{
    void ObjectTracker::rotatorth_run()
    {
        while (rotatorth_should_run)
        {
            ...
            if (rotator_handler && rotator_handler->is_connected())
            {
                ...
                rot_current_req_vel.az = 0.0;
                rot_current_req_vel.el = 0.0;
                ...
                if (rotator_engaged)
                {
                    if (rotator_tracking)
                    {
                        if (sat_current_pos.el > 0)
                        {
                            ...
                            rot_current_req_vel.az = sat_current_vel.az;
                            rot_current_req_vel.el = sat_current_vel.el;

                            if (meridian_flip_correction){
                                rot_current_req_pos.az = correctRotatorAzimuth(rot_current_req_pos.az);
                                rot_current_req_pos.el = correctRotatorElevation(rot_current_req_pos.el);
                            }
                        }
                    ...
                    if (rot_current_reqlast_pos.az != rot_current_req_pos.az || rot_current_reqlast_pos.el != rot_current_req_pos.el){
                        rot_current_req_pos.az = std::clamp(rot_current_req_pos.az, static_cast<float>(rotator_az_min), static_cast<float>(rotator_az_max));
                        rot_current_req_pos.el = std::clamp(rot_current_req_pos.el, static_cast<float>(0), static_cast<float>(110));

                        if (rotator_handler->set_pos(rot_current_req_pos.az,
                                                     rot_current_req_pos.el,
                                                     rot_current_req_vel.az,
                                                     rot_current_req_vel.el) != rotator::ROT_ERROR_OK)
    ...
    {
        float desired_az = az;
        while (desired_az > max_correction_azimuth) desired_az -= 360.0;
        while (desired_az < min_correction_azimuth) desired_az += 360.0;
        return desired_az + az_delta;
    }

    float ObjectTracker::correctRotatorElevation(const float el)
    {
        return el + el_delta;
    }
    ...
    void ObjectTracker::renderRotatorStatus()
    {
        ...
        if (ImGui::BeginTable("##trackingwidgettable", 2, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg))
        {
            ...
            ImGui::TableNextRow();
            ImGui::TableSetColumnIndex(0);
            widgets::AzElInput("##velaz", &rot_current_req_vel.az, false);
            ImGui::TableSetColumnIndex(1);
            widgets::AzElInput("##velel", &rot_current_req_vel.el, false);
    ...
    void ObjectTracker::renderRotatorConfig()
    {
        ...
        widgets::SteppedSliderFloat("Azimuth delta", &az_delta, -2.5, 2.5, 0.1);
        widgets::SteppedSliderFloat("Elevation delta", &el_delta, -2.5, 2.5, 0.1);
        widgets::SteppedSliderFloat("Time Delta", &time_delta, -500, 500, 10, "%.1f ms");

        ImGui::Spacing();
        ImGui::Separator();
        ImGui::Spacing();

        ImGui::InputDouble("Azimuth Angle##Azimuth Angle", &rotator_correction_az);
        ImGui::InputDouble("Tilt Angle##Tilt Angle", &rotator_correction_el);
        ImGui::InputDouble("Tilt Direction##Tilt Direction", &rotator_correction_el_dir);
    ...
    nlohmann::json ObjectTracker::getRotatorConfig()
    {
        ...
        v["rotator_correction_az"] = rotator_correction_az;
        v["rotator_correction_el"] = rotator_correction_el;
        v["rotator_correction_el_dir"] = rotator_correction_el_dir;
    ...
    void ObjectTracker::setRotatorConfig(nlohmann::json v)
    {
        ...
        rotator_correction_az = getValueOrDefault(v["rotator_correction_az"], rotator_correction_az);
        rotator_correction_el = getValueOrDefault(v["rotator_correction_el"], rotator_correction_el);
        rotator_correction_el_dir = getValueOrDefault(v["rotator_correction_el_dir"], rotator_correction_el_dir);
        time_delta = getValueOrDefault(v["time_delta"], time_delta);
    }
}
