...
namespace satdump
{
    void ObjectTracker::renderPolarPlot()
    {
        ...
        draw_list->AddCircle({ImGui::GetCursorScreenPos().x + (d_pplot_size / 2),
                             ...
                             radius1, style::theme.light_green, 0, 1);
        draw_list->AddCircle({ImGui::GetCursorScreenPos().x + (d_pplot_size / 2),
                              ImGui::GetCursorScreenPos().y + (d_pplot_size / 2)},
                             radius2, style::theme.light_green, 0, 1);
        draw_list->AddCircle({ImGui::GetCursorScreenPos().x + (d_pplot_size / 2),
                              ImGui::GetCursorScreenPos().y + (d_pplot_size / 2)},
                             radius3, style::theme.light_green, 0, 1);
        ...
        if(meridian_flip_correction){
          float limits_x1, limits_x2, limits_y1, limits_y2;
          limits_x1 = limits_x2 = ImGui::GetCursorScreenPos().x + (d_pplot_size / 2);
          limits_y1 = limits_y2 = ImGui::GetCursorScreenPos().y + (d_pplot_size / 2);
          limits_x1 += az_el_to_plot_x(d_pplot_size, radius, rotator_az_min, 0);
          limits_y1 -= az_el_to_plot_y(d_pplot_size, radius, rotator_az_min, 0);
          limits_x2 += az_el_to_plot_x(d_pplot_size, radius, rotator_az_min, 90);
          limits_y2 -= az_el_to_plot_y(d_pplot_size, radius, rotator_az_min, 90);
          if(rot_current_req_pos.az < 0) draw_list->AddLine({limits_x1, limits_y1},
                             {limits_x2, limits_y2},
                             style::theme.red, 2.0);
          limits_x1 = limits_x2 = ImGui::GetCursorScreenPos().x + (d_pplot_size / 2);
          limits_y1 = limits_y2 = ImGui::GetCursorScreenPos().y + (d_pplot_size / 2);
          limits_x1 += az_el_to_plot_x(d_pplot_size, radius, rotator_az_max, 0);
          limits_y1 -= az_el_to_plot_y(d_pplot_size, radius, rotator_az_max, 0);
          limits_x2 += az_el_to_plot_x(d_pplot_size, radius, rotator_az_max, 90);
          limits_y2 -= az_el_to_plot_y(d_pplot_size, radius, rotator_az_max, 90);
          if(rot_current_req_pos.az > 0) draw_list->AddLine({limits_x1, limits_y1},
                             {limits_x2, limits_y2},
                             style::theme.red, 2.0);
        }
        ...
        if (upcoming_pass_points.size() > 1)
        {
            ...
            for (int i = 0; i < (int)upcoming_pass_points.size() - 1; i++)
            {
                ...
                ImColor linecolor = style::theme.orange;
                if(p1.az < 0 || p2.az < 0) linecolor = style::theme.fuchsia;
        ...
        if (rotator_handler && rotator_handler->is_connected())
        {
#if 1
            {
                ...
                draw_list->AddCircle({point_x, point_y}, 9 * ui_scale, rot_current_pos.az > 0 ? style::theme.light_cyan : style::theme.magenta, 0, 2.0);
            }
            ...
            if (rotator_engaged)
            {
                ...
                draw_list->AddLine({point_x - 5 * ui_scale, point_y}, {point_x - 12 * ui_scale, point_y}, rot_current_req_pos.az > 0 ? style::theme.light_cyan : style::theme.magenta, 2.0);
                draw_list->AddLine({point_x + 5 * ui_scale, point_y}, {point_x + 12 * ui_scale, point_y}, rot_current_req_pos.az > 0 ? style::theme.light_cyan : style::theme.magenta, 2.0);
                draw_list->AddLine({point_x, point_y - 5 * ui_scale}, {point_x, point_y - 12 * ui_scale}, rot_current_req_pos.az > 0 ? style::theme.light_cyan : style::theme.magenta, 2.0);
                draw_list->AddLine({point_x, point_y + 5 * ui_scale}, {point_x, point_y + 12 * ui_scale}, rot_current_req_pos.az > 0 ? style::theme.light_cyan : style::theme.magenta, 2.0);
            }
        }
#if 1
        if (next_aos_time != 0 && next_los_time != 0)
        {
            ...
            ImGui::TextColored(style::theme.light_green, "%s", obj_name.c_str());
            curs = ImGui::GetCursorScreenPos();
            char buff[9];
            snprintf(buff, sizeof(buff), "Az: %.1f", sat_current_pos.az);
            size = ImGui::CalcTextSize(buff);
            draw_list->AddRectFilled(curs, ImVec2(curs.x + size.x + 2 * style.FramePadding.x, curs.y + size.y), style::theme.overlay_bg);
            ImGui::TextColored(style::theme.light_green, "Az: %.1f", sat_current_pos.az);
            curs = ImGui::GetCursorScreenPos();
            snprintf(buff, sizeof(buff), "El: %.1f", sat_current_pos.el);
            size = ImGui::CalcTextSize(buff);
            draw_list->AddRectFilled(curs, ImVec2(curs.x + size.x + 2 * style.FramePadding.x, curs.y + size.y), style::theme.overlay_bg);
            ImGui::TextColored(style::theme.light_green, "El: %.1f", sat_current_pos.el);
            ...
            curs = ImGui::GetCursorScreenPos();
            draw_list->AddRectFilled(curs, ImVec2(curs.x + size.x + 2 * style.FramePadding.x, curs.y + size.y), style::theme.overlay_bg);
            ImGui::TextColored(style::theme.light_green, "%s in %s", next_aos_time > ctime ? "AOS" : "LOS", time_dis.c_str());
